 ...
 */
 ...
 export const POST: RequestHandler = async (event) => {
   ...
   const {data, error} = await supabaseClient.rpc('is_admin', { user_id: session.user.id })
   if (data) {
 
     const { values } = await event.request.json()
     const userData = { first_name: values.firstName, last_name: values.lastName, access_level: 'academy', school_id: values.schoolId }
